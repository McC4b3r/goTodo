...
build_protos() {
  ...
  PATH="$PATH:$(pwd)/node_modules/.bin"
	cd protos; buf generate;protoc-go-inject-tag -input "gen/go/go_app/v1/*.*.*.go";protoc-go-inject-tag -input "gen/go/go_app/v1/*.*.go";go mod tidy; cd -
	#mkdir -p protos/clients/js; cd protos/clients/js; openapi-generator-cli generate -g javascript -i ../../gen/docs/go_app/v1/api.swagger.json -c config.yaml; npm install; npm run build; cd -
	mkdir -p protos/clients/ts; cd protos/clients/ts; openapi-generator-cli generate -g typescript-fetch -i ../../gen/docs/go_app/v1/api.swagger.json -c config.yaml; npm install; npm run build; cd -
}
